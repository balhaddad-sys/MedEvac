...
function vHome(){
  ...
  return'<div class="screen"><div class="hdr"><div class="hdr-c"><h1>MedEvac</h1><p>Mubarak Al-Kabeer Hospital</p></div><div class="badge '+(_authFailed?"badge-off":S.online?"badge-on":"badge-off")+'"><div class="ldot"></div>'+(_authFailed?"Auth Error":S.online?"Online":"Offline")+'</div><button class="hbtn" id="ba">'+I.cog+'</button></div><div class="sp" style="padding:14px 16px 100px"><div class="hero"><div style="display:flex;justify-content:space-between;margin-bottom:6px;position:relative;z-index:1"><span style="font-size:10px;opacity:.5;font-weight:700;text-transform:uppercase;letter-spacing:1px">Total Patients</span><span style="font-size:9px;opacity:.25;font-weight:500">'+new Date().toLocaleDateString("en",{month:"short",day:"numeric",year:"numeric"})+'</span></div><div style="font-size:44px;font-weight:900;letter-spacing:-2px;position:relative;z-index:1;color:#fff">'+total+'</div><div style="font-size:11px;opacity:.25;margin-top:4px;font-weight:500;position:relative;z-index:1">Across all units</div></div>'+["A","B","C","D","E"].map(u=>'<div class="uc"><div class="uh"><span>Unit '+u+'</span><span>'+(cnt[u+"_M"]+cnt[u+"_F"])+'</span></div><div class="ub"><div class="ubtn female" data-unit="'+u+'_F"><div class="ubtn-ico">'+I.user+'</div><div class="ubtn-name">Female</div><div class="ubtn-cnt">'+cnt[u+"_F"]+'</div></div><div class="ubtn male" data-unit="'+u+'_M"><div class="ubtn-ico">'+I.user+'</div><div class="ubtn-name">Male</div><div class="ubtn-cnt">'+cnt[u+"_M"]+'</div></div></div></div>').join("")+'</div></div>';
}
